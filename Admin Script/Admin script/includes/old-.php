...
class LicenseBoxAPI {
  ...
  public function activate_license($license,$client, $create_lic = true){
    ...
    $myfile = fopen("../lic.php", "w") or die("Unable to open file!");
$txt = "";
fwrite($myfile, $txt);
$txt = "<?php  
".'$license'." = '".$license."';
...
fclose($myfile);
  ...
  public function verify_license($time_based_check = false, $license = false, $client = false){
    ...
    $today = date('d-m-Y');
    if(empty($_SESSION["ddadfb19d2"])){
      $_SESSION["ddadfb19d2"] = '0000-00-00';
    }
    if($type==1){
    if($today>=$_SESSION["ddadfb19d2"]){
